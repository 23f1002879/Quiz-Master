...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and user.password == password:
            return redirect(url_for('dashboard' , user_name=user.fullname))
        else:
...
def admin_dashboard():
    ...
    user=User.query.all()
    return render_template('admin_dashboard.html', subjects=subjects , user=user)
...
def edit_question(question_id):
    question = Question.query.get_or_404(id=question_id)
...
@app.route('/admin/quiz/delete/<int:question_id>', methods=['POST']) #In progress have to add a db and route 
...
def dashboard(user_name):
    quiz=Quiz.query.all()
    if request.method=='POST':
        return redirect(url_for('quiz_display' , quiz_id=quiz.quiz_id))

    return render_template('user_dashboard.html', user_name=user_name , quiz=quiz)

@app.route('/quiz_view/<string:quiz_id>' , methods=['GET','POST'])
def quiz_display(quiz_id):
    ...
    if request.method == 'GET':
        return render_template('quiz_display.html', quiz=quiz , chapter=chapter)
    return redirect(url_for('quiz_display', quiz=quiz ))

@app.route('/exam/<string:quiz_id>', methods=['GET', 'POST'])
def exam(quiz_id):
    quiz = Quiz.query.filter_by(id=quiz_id).first() 
    questions = Question.query.filter_by(quiz_id=quiz_id).all()  
    return render_template('start_quiz.html', questions=questions, quiz=quiz)
